...
$conn = mysqli_connect("localhost", "admin_3", "12345", "logindb");
...
if (isset($_POST) && $_SESSION['user']['role'] == 'adminthree') {
    foreach ($_POST as $key => $value) {
        ...
        $id = explode("_", $key)[1];
        $out = mysqli_query($conn, "UPDATE aproval SET admin_3=$value WHERE id =$id");
    }
}
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post" action="adminthree.php">
                                <div>
                                    <button class="btn btn-success" name="1 <?php echo ($row["id"]) ?>" class="w3-button w3-green" <?php echo ($row["admin_3"] == 1) ? 'disabled' : '' ?>>APPROVE</button>
                                    <button class="btn btn-danger" name="-1 <?php echo ($row["id"]) ?>" class="w3-button w3-red" <?php echo ($row["admin_3"] == -1) ? 'disabled' : '' ?>>REJECT</button>
                                </div>
                            </form>
                        </td>
                    </tr>

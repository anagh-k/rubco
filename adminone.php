...
if (isset($_POST) && $_SESSION['user']['role'] == 'adminone') {
            ...
            <tbody>
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <th bgcolor="<?php echo (($row["admin_1"] == 1) ? 'green' : (($row["admin_1"] == -1) ? 'red' : 'white')) ?>"><?php echo (($row["admin_1"] == 1) ? 'APPROVED' : (($row["admin_1"] == -1) ? 'REJECTED' : 'PENDING')) ?></th>
                        <th>
                            <form method="post" action="adminone.php">
                                <div class="w3-section">
                                    <button name="1 <?php echo ($row["id"]) ?>" class="w3-button w3-green" <?php echo ($row["admin_1"] == 1) ? 'disabled' : '' ?>>APPROVE</button>
                                    <button name="-1 <?php echo ($row["id"]) ?>" class="w3-button w3-red" <?php echo ($row["admin_1"] == -1) ? 'disabled' : '' ?>>REJECT</button>
                                </div>
                            </form>
                        </th>
                    </tr>
